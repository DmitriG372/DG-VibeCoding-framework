...
Create a new sprint from a plan, backlog, or user description. Generates a validated `sprint/sprint.json` (sprint-v3 contract) and triggers `sprint/sprint.md` generation via hook.
...
| `steps` | 5–10 `{ id, desc, done }` objects | Mandatory decomposition unless trivial |
| `corridor` | `{ allowed, forbidden }` | Explicit file-scope glob patterns |
| `trivial` | Boolean | True only for a change below 50 LOC |
| `complexity` | `trivial` / `low` / `medium` / `high` | Scope estimate |
...
Kuidas tööd korraldada?
  1. sequential — üks agent korraga samas checkout'is (vaikimisi)
  2. worktree — paralleelne partner saab alati eraldi worktree
...
- Write `sprint/sprint.json` with the sprint-v3 contract (see below)
- Run `node scripts/validate-sprint.js sprint/sprint.json --write-stats`
...
## Output: sprint/sprint.json (sprint-v3)
  ...
  "features": [
    {
      ...
      "steps": [
        { "id": "S1", "desc": "Inspect existing behavior", "done": false },
        { "id": "S2", "desc": "Write a failing test", "done": false },
        { "id": "S3", "desc": "Implement the change", "done": false },
        { "id": "S4", "desc": "Run targeted verification", "done": false },
        { "id": "S5", "desc": "Run the full relevant suite", "done": false }
      },
...
- **Complexity is an estimate** -- used by `/feature` for agent routing (low -> implementer, high -> plan-checker -> orchestrator -> implementer)
